	...
	db.transaction(() => {
		const tracks = db
			.query<
				...
				[string]
			>('SELECT id FROM application_user_track WHERE userId = ?')
			.all(id);
		for (const track of tracks) {
			db.run('DELETE FROM application_user_step WHERE userTrackId = ?', [track.id]);
		}
		db.run('DELETE FROM application_user_track WHERE userId = ?', [id]);
		db.run('DELETE FROM user_event WHERE userId = ?', [id]);
	})();

	Auth.invalidateSessions(locals, id);
	db.run('DELETE FROM user WHERE id = ?', [locals.session.userId]);
