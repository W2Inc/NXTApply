<script lang="ts">
	...
	import { BadgeCheck, BadgeX, Building2, Calendar, CheckCircle, CircleAlert, ExternalLink, MapPin } from '@lucide/svelte';
	import { cn } from '$lib/utils';
	import { Button } from '$lib/components/ui/button';
	import type { AvailableUserEvent } from './+page.server';
	import { FormKit } from '$lib/form.svelte';
	import { joinEvent } from './app.remote';
	import { Badge } from '$lib/components/ui/badge';

	const { event }: { event: AvailableUserEvent } = $props();
	export const formatter = new DateFormatter(page.data.locale, {
		year: 'numeric',
		month: 'long',
		day: 'numeric',
		hour: '2-digit',
...
<li class="flex flex-col gap-4 py-3 first:pt-0 last:pb-0 sm:flex-row sm:items-center">
	...
	{#if event.trackId || !event.userEventId}
		<form class="mt-2 flex items-center gap-2 sm:mt-0" {...FormKit.remote(joinEvent)}>
