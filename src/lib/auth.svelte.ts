...
export namespace Auth {
	 ...
	 */
	export async function invalidateSession(locals: App.Locals, id: string) {
		locals.db.query<Session, string>(`DELETE FROM session WHERE id = ?`).run(id);
	}
	 ...
	 */
	export async function invalidateSessions(locals: App.Locals, userId: string) {
		locals.db.query<Session, string>(`DELETE FROM session WHERE userId = ?`).run(userId);
	}
	...
	export async function createResetToken(locals: App.Locals, userId: string) {
		...
		locals.db.transaction(() => {
			locals.db.query(`DELETE FROM reset_token WHERE userId = ?`).run(userId);
			locals.db
				.query(`INSERT INTO reset_token (id, userId, expiresAt) VALUES (?, ?, ?)`)
				.run(tokenId, userId, expiresAt.getTime());
		})();
