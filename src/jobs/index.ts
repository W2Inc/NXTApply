...
namespace Jobs {
	...
	export function create(name: string, smol: boolean = true) {
		...
		Logger.inf(`[Worker]: '${name}' started`);
		worker.onmessage = (event) => {
			if ('message' in event.data) {
				return Logger.inf(`[Worker]: '${name}' message: ${event.data.message}`);
			}
			// Log.info(event.data, `[Worker] '${name}' message:`);
		};

		worker.onerror = (error) => {
			Logger.err(`[Worker]: '${name}' error: ${error.message}`);
		};

		worker.onmessageerror = (error) => {
			Logger.err(`[Worker]: '${name}' message error: ${error}`);
		};
